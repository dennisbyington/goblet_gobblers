"""Search functions for games.  Based on the textbook "Artificial Intelligence: A Modern Approach (Chapter 5 - Adversarial Search)"""
...
def alpha_beta_cutoff_search(state, game, d=4, cutoff_test=None, eval_fn=None):
    ...
    eval_fn = eval_fn or (lambda state, player: game.min_max_value(state, player))
    best_score = -np.inf
    beta = np.inf
    best_action = None
    for a in game.actions(state):
